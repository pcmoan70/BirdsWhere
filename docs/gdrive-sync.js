...
window.GDriveSync = (function () {
  ...
  async function sync() {
    ...
    try {
      ...
      // Scalar-settings direction (collections always union regardless): remote
      // wins ONLY when its change-stamp is strictly newer than what THIS device
      // last saved AND the user hasn't edited locally this session. Every payload
      // we write carries updatedAt (GeoState.write stamps it), so a remote with a
      // missing/0 stamp is legacy/foreign and an equal stamp is same-origin — both
      // fall to local-wins, which never clobbers current-device settings.
      ...
      // trigger a full resumable upload on every sync.
